...
/* INTERNAL REFERENCE */
using DATECS_EXELLIO_FPU550.DriverUI;
using DATECS_EXELLIO_FPU550.Config;
using DATECS_EXELLIO_FPU550.DriverUI.Customs;

namespace DATECS_EXELLIO_FPU550
{
    [PluginSimpleAttribute(PluginType.FPDriver)]
    public class DATECS_EXELLIO_FPU550 : IFPDriver
    {
        // UI Components
        ...
        private void FP_Sale(object[] param)
        {
            ...
            try
            {
                ...
                string[] _ain444fo = SetGetArticle('R', nexArticleNo);
                if (_ain444fo.Length == 0 || _ain444fo[0] == "F")
                {//ok
                    //nexArticleNo++;
                }
                else
                {
                    _ain444fo = SetGetArticle('F');
                    bool tru = true;
                    while (tru)
                    {
                        _ain444fo = SetGetArticle('N');
                        if (_ain444fo.Length == 0 || _ain444fo[0] == "F")
                            tru = false;
                        else nexArticleNo = uint.Parse(_ain444fo[1]);
                    }
                }
                ...
                for (int i = 0; i < dTable.Rows.Count; i++)
                {
                    ...
                    _ain444fo = SetGetArticle('R', nexArticleNo);
                    if (_ain444fo.Length != 0 && _ain444fo[0] != "F")
                    {
                        truee = true;
                        while (truee)
                        {
                            _ain444fo = SetGetArticle('R');
                            if (_ain444fo.Length == 0 || _ain444fo[0] == "F")
                                truee = false;
                            else
                                nexArticleNo++;
        ...
        private void FP_SetCashier(object[] param)
        {
            // Return if parameters are empty
            if (func.IsEmpty(param))
                return;
