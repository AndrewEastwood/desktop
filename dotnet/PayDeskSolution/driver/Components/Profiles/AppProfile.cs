    ...
    public class AppProfile
    {
        ...
        public void calcCash()
        {
            ...
            else
            {
                ...
                Cash[CoreConst.DISC_ONLY_PERCENT] = MathLib.GetRoundedMoney(_discArrC[0] + _discArrC[1]);
            ...
            for (i = 0; i < saleItems.Rows.Count; i++)
            {
                ...
                {
                    newTmpPrice = MathLib.GetDouble(saleItems.Rows[i]["PR" + this.clientPriceNo].ToString());
                ...
                saleItems.Rows[i]["ASUM"] = Cheque.Rows[i]["SUM"] = MathLib.GetRoundedMoney(MathLib.GetDouble(saleItems.Rows[i]["TOT"].ToString()) * newPrice);
            ...
            DataRow[] prRows = null;
            if (this.clientPriceNo != 0)
                prRows = saleItems.Select("PR" + this.clientPriceNo + " <> 0");
        ...
        public Hashtable GetCashStructure()
        {
            Hashtable cashStructure = new Hashtable();
