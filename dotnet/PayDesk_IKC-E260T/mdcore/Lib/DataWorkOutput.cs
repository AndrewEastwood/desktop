...
namespace mdcore.Lib
{
    ...
    public static class DataWorkOutput
    {
        ...
        public static void Print(string name, DataTable order)
        {
            foreach (KeyValuePair<string, Dictionary<string, string>> printer in AppConfig.Path_Printers)
            {
                if (printer.Value["NAME"].ToString() == name)
                {
                    ...
                    catch (Exception ex) { CoreLib.WriteLog(ex, "mdcore.Lib.DataWorkOutput.Print(string name, DataTable order);"); }
                }
            }
        }
                ...
                if (fPath != string.Empty && printer.Value["PRN"] != string.Empty)
                {
                    ...
                    if (timeoutIs)
                    {
                        ...
                        //foreach (System.Diagnostics.Process sp in pp)
                        {
                            ...
                            winapi.WApi.SetForegroundWindow((int)(pp[0].MainWindowHandle));
                            System.Drawing.Rectangle msgRect = new System.Drawing.Rectangle();
                            winapi.WApi.GetWindowRect(pp[0].MainWindowHandle, ref msgRect);

                            mdcore.Components.UI.MMessageBox.StartPoint = new System.Drawing.Point(msgRect.Location.X - 50, msgRect.Location.Y - 150);
                            mdcore.Components.UI.MMessageBox.Show("Закрийте попереднє вікно друку, якщо таке є.\r\nПотім закрийте це повідомлення для нового друку.", System.Windows.Forms.Application.ProductName);
                    ...
                    if (isBusy)
                        mdcore.Components.UI.MMessageBox.Show("Закрийте попереднє вікно програми друку для наступного друкування документу", "Додатковий друк - InTech PayDesk");
                    */
                    System.Diagnostics.Process.Start(printer.Value["PRN"], fPath);
                    fRez = true;
                }
            }
            catch (Exception ex) { CoreLib.WriteLog(ex, "mdcore.Lib.DataWorkOutput.PrintThrowPrinter(object printerConfig, DataTable order);"); }
        ...
        {   //input data
            ...
            object[] simpleData = new object[22];
            ...
            catch (Exception e) { CoreLib.WriteLog(e, "mdcore.Lib.DataWorkOutput.FeetchTemplate(string tplFile, Dictionary<string, object> data) Attempt to initialize: Basic Information"); }
            ...
            if (data.ContainsKey("PAYMENT") && data["PAYMENT"] != null && data["PAYMENT"] != string.Empty)
            {
                ...
                catch (Exception e) { CoreLib.WriteLog(e, "mdcore.Lib.DataWorkOutput.FeetchTemplate(string tplFile, Dictionary<string, object> data) Attempt to initialize: Payment Information"); }
            }
            ...
            if (data.ContainsKey("DISCOUNT") && data["DISCOUNT"] != null && data["DISCOUNT"] != string.Empty)
            {
                ...
                try
                {
                    Dictionary<string, object> discountInfo = (Dictionary<string, object>)data["DISCOUNT"];
                ...
                catch (Exception e) { CoreLib.WriteLog(e, "mdcore.Lib.DataWorkOutput.FeetchTemplate(string tplFile, Dictionary<string, object> data) Attempt to initialize: Discount Information"); }
            }
            ...
            if (data.ContainsKey("BILL") && data["BILL"] != null)
            {
                ...
                catch (Exception e) { CoreLib.WriteLog(e, "mdcore.Lib.DataWorkOutput.FeetchTemplate(string tplFile, Dictionary<string, object> data) Attempt to initialize: Bill Information"); }
            ...
            catch (Exception e) { CoreLib.WriteLog(e, "mdcore.Lib.DataWorkOutput.FeetchTemplate(string tplFile, Dictionary<string, object> data) Attempt to perform Base Function: mdcore.Lib.DataWorkOutput.FeetchTemplate(string tplFile, object[] data)"); }
                    ...
                    try
                    {
                        ...
                        switch (separator[i])
                        {
                            case "[hex:":
                                ...
                                break;
                            case "[dec:":
                                newData += (char)Convert.ToByte(byte.Parse(val));
