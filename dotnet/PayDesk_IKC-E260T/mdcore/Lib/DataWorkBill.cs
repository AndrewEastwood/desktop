...
namespace mdcore.Lib
{
    public static class DataWorkBill
    {
        ...
        public static bool BillUpdatePrintedCount(DataTable dtBill)
        {
            ...
            for (int i = 0; i < dtBill.Rows.Count; i++)
                try
                {
                    ...
                    CoreLib.WriteLog(e, "mdcore.Lib.BillUpdatePrintedCount(DataTable dtBill); can't update PRINTCOUNT field at row ["+i+"]\r\n" + DataWorkShared.DumpDataTableRow(dtBill.Rows[i]));
                    success = false; 
                }
            return success;
        }
        public static bool BillUpdatePrintedCount(DataTable dtBill, bool saveResult)
        {
            ...
            {
                CoreLib.WriteLog(e, "mdcore.Lib.BillUpdatePrintedCount(DataTable dtBill, bool saveResult);");
                success = false;
            }
            return success;
        }
        ...
        {
            return mdcore.Lib.SecureLib.GetHashMD5(DateTime.Now.ToString() + "_" + newBillNumber);
        }
            ...
            */
                try
                {
                    ...
                    if (isNewBill)
                    {
                        ...
                        do
                        {
                            path = string.Format("{0:X2}_N{1}{3}_{2:ddMMyy}.bill", AppConfig.APP_SubUnit, nom.PadLeft(5, '0'), DateTime.Now, i != 0 ? "-" + i : "");
                            nom = string.Format("{0}{1}", nom.PadLeft(5, '0'), i != 0 ? "-" + i : "");
                            i++;
                        } while (File.Exists(AppConfig.Path_Bills + "\\" + path));
                        ...
                        billInfo[mdcore.Common.CoreConst.OWNER_NO] = string.Empty;
                    ...
                    else
                    {
                        ...
                        {
                            DataWorkShared.SetBillProperty(dTable, mdcore.Common.CoreConst.COMMENT, comment);
                            //billInfo["COMMENT"] = comment;
                        }
                    }
                    ...
                    if (isNewBill)
                    {
                        ...
                        StreamWriter sw = new StreamWriter(AppConfig.Path_Bills + "\\" + "base.dat");
        ...
        */
        ...
        {
            DataWorkShared.SetBillProperty(dtBill, mdcore.Common.CoreConst.IS_LOCKED, true);
            //dtBill.ExtendedProperties["LOCK"] = true;
            if (fxNo != string.Empty)
                DataWorkShared.SetOrderProperty(dtBill, mdcore.Common.CoreConst.ORDER_NO, fxNo);
            SaveBillToFile(dtBill);
        }
        ...
        public static void UnlockBill(DataTable dtBill)
        {
            ...
            DataWorkShared.SetBillProperty(dtBill, mdcore.Common.CoreConst.IS_LOCKED, false);
            SaveBillToFile(dtBill);
        }
        ...
        public static void SaveBillToFile(DataTable dtBill)
        {
            ...
            FileStream stream = new FileStream(AppConfig.Path_Bills + "\\" + DataWorkShared.ExtractBillProperty(dtBill, mdcore.Common.CoreConst.PATH) , FileMode.OpenOrCreate);
            BinaryFormatter binF = new BinaryFormatter();

            DataWorkShared.SetBillProperty(dtBill, mdcore.Common.CoreConst.DATETIMEEDIT, DateTime.Now);
        ...
        public static void MadeBillCopy(DataTable dtBill)
        {
            ...
            string nomOrig = DataWorkShared.ExtractBillProperty(dtBill, mdcore.Common.CoreConst.BILL_NO, string.Empty).ToString();
            string nom = nomOrig;
            ...
            {
                preffix++;
                path = string.Format("{0:X2}_N{1}{3}_{2:ddMMyy}.bill", AppConfig.APP_SubUnit, nom, DateTime.Now, preffix != 0 ? "-" + preffix : "");
            ...
            DataWorkShared.SetBillProperty(dtBill, mdcore.Common.CoreConst.OID, BillNewUID(newNom));
            DataWorkShared.SetBillProperty(dtBill, mdcore.Common.CoreConst.BILL_NO, newNom);
            DataWorkShared.SetBillProperty(dtBill, mdcore.Common.CoreConst.OWNER_NO, nomOrig);
            DataWorkShared.SetBillProperty(dtBill, mdcore.Common.CoreConst.DATETIME, DateTime.Now.ToShortDateString());
            DataWorkShared.SetBillProperty(dtBill, mdcore.Common.CoreConst.PATH, path);
        ...
        {
            /*
             if (DataWorkShared.GetOrderInfo(dtBill)[mdcore.Common.CoreConst.ORDER_NO] != string.Empty)
             {
                 MMessageBox.Show("Рахунок № " + DataWorkShared.ExtractBillProperty(dtBill, mdcore.Common.CoreConst.BILL_NO) + " є закритий",
                     Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                 return false;
             }

             if (MMessageBox.Show("Видалити рахунок № " + DataWorkShared.ExtractBillProperty(dtBill, mdcore.Common.CoreConst.BILL_NO),
            ...
            */
            ...
            try
            {
                Microsoft.VisualBasic.FileIO.FileSystem.DeleteFile(AppConfig.Path_Bills + "\\" + DataWorkShared.ExtractBillProperty(dtBill, mdcore.Common.CoreConst.PATH), UIOption.OnlyErrorDialogs, RecycleOption.SendToRecycleBin);
                dtBill.Clear();
        ...
        public static Dictionary<string, object> LoadDayBills(DateTime selectedDay, string path, byte subunit)
        {
            ...
            for (int i = 0; i < bills.Length; i++/*, item = string.Empty*/)
            {
                try
                {
                    ...
                    billSuma = (double)props[mdcore.Common.CoreConst.ORDER_SUMA];
