...
namespace mdcore.Components.Objects
{
    public class ComPort : IDisposable
    {
        ...
        public ComPort()
        {
            ...
            handle = IntPtr.Zero;
            ReadIntervalTimeout = winapi.WApi.MAXDWORD;
        ...
        public bool Available(int portIndex)
        {
            ...
            IntPtr tmphPort = WApi.CreateFile(portName,
                (UInt32)dwDesiredAccess.GENERIC_READ, 0, IntPtr.Zero,
                (UInt32)dwCreationDisposion.OPEN_EXISTING, 0x0, IntPtr.Zero);
            if (tmphPort == (IntPtr)WApi.INVALID_HANDLE_VALUE)
                return false;
            else
                WApi.CloseHandle(tmphPort);
        ...
        public bool Read(ref byte[] buffer, out uint count)
        {
            ...
            bool fOK = false;
            OLRead.hEvent = WApi.CreateEvent(IntPtr.Zero, true, false, null);

            if (OLRead.hEvent != null)
            {
                fOK = WApi.ReadFile(handle, buffer, (uint)buffer.Length, out count, ref OLRead);
                if (!fOK)
                {
                    if (Marshal.GetLastWin32Error() == WApi.ERROR_IO_PENDING)
                    {
                        fOK = (WApi.WaitForSingleObject(OLRead.hEvent, 5000) == WApi.WAIT_OBJECT_0) &&
                            WApi.GetOverlappedResult(handle, ref OLRead, ref count, false);
                    }
                }
                WApi.CloseHandle(OLRead.hEvent);
                if (!fOK)
                    PortClear();
            }
            else
        ...
        {
            if (handle == (IntPtr)WApi.INVALID_HANDLE_VALUE) return false;
            ...
            bool fOK = false;
            OverlappedWrite.hEvent = WApi.CreateEvent(IntPtr.Zero, true, false, null);

            if (OverlappedWrite.hEvent != null)
            {
                fOK = WApi.WriteFile(handle, SendArr, (uint)SendArr.Length, out BytesWritten, ref OverlappedWrite);
                if (!fOK && Marshal.GetLastWin32Error() == WApi.ERROR_IO_PENDING)
                {
                    fOK = (WApi.WaitForSingleObject(OverlappedWrite.hEvent, 1000) == WApi.WAIT_OBJECT_0) &&
                        WApi.GetOverlappedResult(handle, ref OverlappedWrite, ref BytesWritten, false);
                }
                if (fOK)
                    fOK = (BytesWritten == SendArr.Length);
                if (!fOK)
                    PortClear();
                WApi.CloseHandle(OverlappedWrite.hEvent);
            }
            else
                Marshal.GetLastWin32Error();
        ...
        public bool PortClear()
        {
            ...
            if (!WApi.PurgeComm(handle, (uint)(WApi.PURGE_TXCLEAR | WApi.PURGE_RXCLEAR)))
                return false;
        ...
        public bool PortAbort()
        {
            ...
            if (!WApi.PurgeComm(handle, (uint)(WApi.PURGE_TXABORT | WApi.PURGE_RXABORT)))
                return false;
        ...
        public bool PortMask(ushort mask)
        {
            Marshal.GetLastWin32Error();
            GC.SuppressFinalize(this);

            if (!WApi.SetCommMask(handle, (UInt16)mask))
                return false;
        ...
        public bool Close()
        {
            isOpen = !WApi.CloseHandle(handle);
        ...
        public static Hashtable GetXmlPortConfiguration(string xmlFileName)
        {
            Hashtable _xml = new Hashtable();

            _xml["PORTINDEX"] = 1;
            _xml["PORT"] = "COM" + 1;// this.portIndex;
            _xml["RATE"] = (int)9600;// this.baudRate;
            _xml["PARITY"] = (byte)0;//this.parity;
            _xml["DBITS"] = (byte)8;// this.dataBits;
            _xml["SBITS"] = (byte)1;// this.stopBits;
            _xml["RT"] = 0;// this.ReadIntervalTimeout;
            _xml["RM"] = 0;// this.ReadTotalTimeoutMultiplier;
            _xml["RC"] = 0;// this.ReadTotalTimeoutConstant;
            _xml["WM"] = 0;//this.WriteTotalTimeoutMultiplier;
            _xml["WC"] = 0;// this.WriteTotalTimeoutConstant;
            _xml["TAG"] = "";// this.WriteTotalTimeoutConstant;

            FileInfo fi = new FileInfo(xmlFileName);
            if (!fi.Exists)
            {
                System.Windows.Forms.MessageBox.Show("Неможливо завантажити конфігурацію порту з файлу " + xmlFileName,
                    "COM порт", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error);
                return _xml;
            }
            ...
            try
            {
                xtr = new XmlTextReader(xmlFileName);
                xtr.WhitespaceHandling = WhitespaceHandling.None;
                while (xtr.Read())
                {
                        ...
                        switch (xtr.Name)
                        {
                            case "PortIndex":
                                ...
                                if (xtr.NodeType == XmlNodeType.Text)
                                    _xml["PORTINDEX"] = int.Parse(xtr.Value);
                                break;
                            case "BaudRate":
                                xtr.Read();
                                if (xtr.NodeType == XmlNodeType.Text)
                                    _xml["RATE"] = int.Parse(xtr.Value);
                                break;
                            case "Parity":
                                xtr.Read();
                                if (xtr.NodeType == XmlNodeType.Text)
                                    _xml["PARITY"] = int.Parse(xtr.Value);
                                break;
                            case "DataBits":
                                xtr.Read();
                                if (xtr.NodeType == XmlNodeType.Text)
                                    _xml["DBITS"] = int.Parse(xtr.Value);
                                break;
                            case "StopBits":
                                xtr.Read();
                                if (xtr.NodeType == XmlNodeType.Text)
                                    _xml["SBITS"] = int.Parse(xtr.Value);
                                break;
                            case "ReadIntervalTimeout":
                                xtr.Read();
                                if (xtr.NodeType == XmlNodeType.Text)
                                    _xml["RT"] = UInt32.Parse(xtr.Value);
                                break;
                            case "ReadTotalTimeoutMultiplier":
                                xtr.Read();
                                if (xtr.NodeType == XmlNodeType.Text)
                                    _xml["RM"] = UInt32.Parse(xtr.Value);
                                break;
                            case "ReadTotalTimeoutConstant":
                                xtr.Read();
                                if (xtr.NodeType == XmlNodeType.Text)
                                    _xml["RC"] = UInt32.Parse(xtr.Value);
                                break;
                            case "WriteTotalTimeoutMultiplier":
                                xtr.Read();
                                if (xtr.NodeType == XmlNodeType.Text)
                                    _xml["WM"] = UInt32.Parse(xtr.Value);
                                break;
                            case "WriteTotalTimeoutConstant":
                                xtr.Read();
                                if (xtr.NodeType == XmlNodeType.Text)
                                    _xml["WC"] = UInt32.Parse(xtr.Value);
                                break;
                            case "Tag":
                                xtr.Read();
                                if (xtr.NodeType == XmlNodeType.Text)
